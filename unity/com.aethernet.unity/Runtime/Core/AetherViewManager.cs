...
namespace AetherNet
{
    ...
    public sealed class AetherViewManager : MonoBehaviour, IFullCollisionSink
    {
        ...
        private void Awake()
        {
            if (Instance != null && Instance != this) { Debug.LogWarning("[AetherNet] Multiple AetherViewManager instances detected."); Destroy(gameObject); return; }
            Instance = this;
            var config = new WorldConfig { Gravity = MathBridge.ToNumerics(_gravity), AllowSleeping = _allowSleeping, MaxBodies = _maxBodies };
...
#endif
            for (int i = 0; i < rigidbodies.Length; i++) RegisterBody(rigidbodies[i], i);
        }

        private void RegisterBody(AetherRigidbody rb, int entityId)
        {
            if (_bodyCount >= _maxBodies) { Debug.LogError($"[AetherNet] MaxBodies exceeded."); return; }
            var def = new BodyDef { BodyType = rb.BodyType, Position = MathBridge.WorldToSim(rb.transform.position), Angle = MathExtensions.ToSimAngle(rb.transform.eulerAngles.z), LinearDamping = rb.LinearDamping, AngularDamping = rb.AngularDamping, GravityScale = rb.GravityScale, FixedRotation = rb.FixedRotation, Constraints = rb.Constraints };
            var body = _physicsWorld.CreateBody(in def, entityId);
            var colliders = rb.GetComponents<IAetherColliderProvider>();
            for (int c = 0; c < colliders.Length; c++) colliders[c].AttachToBody(body, _physicsWorld);
        ...
        {
            for (int i = 0; i < _maxBodies; i++) { AetherRigidbody? rb = _bodies[i]; if (rb == null) continue; var s = _physicsWorld.GetBodyState(i); rb.SnapshotPreTick(in s.Position, s.Angle); }
        ...
        private void LateUpdate()
        {
            ...
            for (int i = 0; i < _maxBodies; i++) { AetherRigidbody? rb = _bodies[i]; if (rb == null) continue; rb.ApplyInterpolatedTransform(alpha); }
        }

        public int RegisterDynamic(AetherRigidbody rb)
        {
            int entityId = -1;
            for (int i = 0; i < _maxBodies; i++) { if (_bodies[i] == null) { entityId = i; break; } }
        ...
        public void Unregister(int entityId)
        {
            ...
            _bodies[entityId] = null; _collisionHandlers[entityId] = null; _triggerHandlers[entityId] = null;
        }

        void ICollisionEnterSink.OnCollisionEnter(ref CollisionData d) { _collisionHandlers[d.EntityIdA]?.OnCollisionEnter(ref d); _collisionHandlers[d.EntityIdB]?.OnCollisionEnter(ref d); }
        void ICollisionExitSink .OnCollisionExit (ref CollisionData d) { _collisionHandlers[d.EntityIdA]?.OnCollisionExit(ref d);  _collisionHandlers[d.EntityIdB]?.OnCollisionExit(ref d); }
        void ITriggerEnterSink  .OnTriggerEnter  (ref TriggerData   d) { _triggerHandlers[d.TriggerEntityId]?.OnTriggerEnter(ref d); _triggerHandlers[d.OtherEntityId]?.OnTriggerEnter(ref d); }
        void ITriggerExitSink   .OnTriggerExit   (ref TriggerData   d) { _triggerHandlers[d.TriggerEntityId]?.OnTriggerExit(ref d);  _triggerHandlers[d.OtherEntityId]?.OnTriggerExit(ref d); }
    }
}
