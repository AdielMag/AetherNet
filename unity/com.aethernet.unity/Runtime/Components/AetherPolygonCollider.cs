...
namespace AetherNet
{
    ...
    public sealed class AetherPolygonCollider : MonoBehaviour, IAetherColliderProvider
    {
        ...
        private void Awake()
        {
            ...
            { var v = MathBridge.ToNumerics(_vertices[i]) / SimulationConstants.PixelsPerMeter; _simVertices.Add(AetherInterop.ToAether(v)); }
        }

        void IAetherColliderProvider.AttachToBody(Body body, PhysicsWorldManager world)
        {
            if (_simVertices == null || _simVertices.Count < 3) { Debug.LogWarning($"[AetherNet] AetherPolygonCollider on '{name}' has fewer than 3 vertices — skipped."); return; }
            ...
            fixture.Friction = friction; fixture.Restitution = restitution; fixture.IsSensor = _isTrigger;
