...
public sealed class ContactTracker
{
    ...
    public bool TryAddNew(int idA, int idB, uint tick, out bool isNew)
    {
        ...
        if (_solidOccupied[slot]) { isNew = false; return true; }
        _solidKeys[slot] = key; _solidOccupied[slot] = true; _solidCount++; isNew = true; return true;
    }
    ...
    public bool Remove(int idA, int idB)
    {
        ...
        _solidOccupied[slot] = false; _solidKeys[slot] = 0; _solidCount--; return true;
    }
    ...
    public bool TryAddTrigger(int idA, int idB, uint tick, out bool isNew)
    {
        ...
        if (slot < 0) { isNew = false; return false; }
        if (_triggerOccupied[slot]) { isNew = false; return true; }
        _triggerKeys[slot] = key; _triggerOccupied[slot] = true; _triggerCount++; isNew = true; return true;
    }
    ...
    public bool RemoveTrigger(int idA, int idB)
    {
        ...
        if (slot < 0) return false;
        _triggerOccupied[slot] = false; _triggerKeys[slot] = 0; _triggerCount--; return true;
    }

    private int FindSlot(ulong[] keys, bool[] occupied, ulong key)
    {
        int hash = (int)((key ^ (key >> 32)) & 0x7FFFFFFF);
        int start = hash % _capacity, current = start;
        do { if (!occupied[current] || keys[current] == key) return current; current = (current + 1) % _capacity; } while (current != start);
        return -1;
    }
    ...
    private int FindExisting(ulong[] keys, bool[] occupied, ulong key)
    {
        int hash = (int)((key ^ (key >> 32)) & 0x7FFFFFFF);
        int start = hash % _capacity, current = start;
        do { if (occupied[current] && keys[current] == key) return current; if (!occupied[current]) return -1; current = (current + 1) % _capacity; } while (current != start);
        return -1;
    }

    private static ulong MakeKey(int idA, int idB)
    {
        int lo = idA < idB ? idA : idB, hi = idA < idB ? idB : idA;
