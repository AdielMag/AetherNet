...
public sealed class SnapshotBuffer
{
    public sealed class AuthoritativeSnapshot
    {
        ...
        public bool          IsValid;
        public AuthoritativeSnapshot(int capacity) => States = new EntityState[capacity];
    }
    ...
    public void Write(uint tick, float timestamp, EntityState[] states, int count)
    {
        ...
        var s = _slots[slot];
        s.TickNumber = tick; s.Timestamp = timestamp; s.EntityCount = count; s.IsValid = true;
        System.Array.Copy(states, s.States, count);
    }

    public bool TryGetBracketing(float renderTime, out AuthoritativeSnapshot? before, out AuthoritativeSnapshot? after)
    {
        before = after = null;
        if (_count < 2) return false;
        for (int i = 0; i < _count - 1; i++)
        {
            int idxA = (_head + i) % _capacity, idxB = (_head + i + 1) % _capacity;
            if (_slots[idxA].Timestamp <= renderTime && _slots[idxB].Timestamp >= renderTime)
            { before = _slots[idxA]; after = _slots[idxB]; return true; }
        }
        return false;
    }

    public AuthoritativeSnapshot? Latest => _count == 0 ? null : _slots[(_head + _count - 1) % _capacity];
}
