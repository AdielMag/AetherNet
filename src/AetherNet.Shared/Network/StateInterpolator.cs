...
public sealed class StateInterpolator
{
    ...
    public int Sample(float serverTime, EntityState[] output)
    {
        ...
        if (!_buffer.TryGetBracketing(renderTime, out var before, out var after) || before == null || after == null)
        {
        ...
        float alpha = duration < 0.0001f ? 1f : (renderTime - before.Timestamp) / duration;
    ...
    {
        for (int i = 0; i < count; i++) if (arr[i].EntityId == entityId) return i;
        return -1;
    }

    private static SNV2 Lerp(SNV2 a, SNV2 b, float t) => new SNV2(a.X + (b.X - a.X) * t, a.Y + (b.Y - a.Y) * t);
}
